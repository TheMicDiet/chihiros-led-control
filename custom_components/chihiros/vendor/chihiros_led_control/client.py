...
class ChihirosMagStirrer(ChihirosDosingPump):
    """Concrete BLE client for a Chihiros magnetic stirrer (``DYMIXR``).

    Reverse engineering of the vendor app (My Chihiros 2.8.59) shows the
    stirrer speaks the dosing-pump protocol verbatim: its channels are
    programmed in timer mode through the same ``dosingSet``/``dosingWorkNew``
    frames, where each timer point's "volume" encodes the stir workload at
    the pump's 0.6 mL/min equivalence. Speed and pre-stir time ride on the
    stirrer-specific ``(0xA5, 42)`` frame; manual start/stop uses the shared
    ...
    async def stir(self, channel: int, on: bool, *, seconds: int | None = None) -> None:
        ...
        """
        cmd = commands.create_general_temp_run_command(self.get_next_msg_id(), {channel: on}, seconds)
        await self._send_command(cmd, 3)
    ...
    async def set_stir_schedule(
        ...
        *,
        ...
        if active:
            commands_to_send.extend(
                commands.create_dosing_schedule_command(
                    self.get_next_msg_id(), channel, commands.DosingMode.TIMER, points
                )
            )
        await self._send_command(commands_to_send, 3)
