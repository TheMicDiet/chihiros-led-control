...
# Nordic UART (same as the LEDs)
UART_SERVICE = "6E400001-B5A3-F393-E0A9-E50E24DCCA9E"
UART_RX      = "6E400002-B5A3-F393-E0A9-E50E24DCCA9E"  # write
UART_TX      = "6E400003-B5A3-F393-E0A9-E50E24DCCA9E"  # notify

# Manual dose (framed command proven by the LED protocol docs in your repo)
# Frame: [CMD, 0x01, LEN, msg_hi, msg_lo, MODE, params..., checksum]
# CMD  = 0xA5, MODE = 0x1B, params = [ch0-3, 0x00, 0x00, ml_hi, ml_lo]
...
def _next_msg_id() -> Tuple[int, int]:
    ...
    if lo == 255:
        if hi == 255:
            new = (0, 1)
        elif hi == 89:  # never become 90
            new = (hi + 2, 0)
        else:
            new = (hi + 1, 0)
    else:
        if lo == 89:    # never become 90
            new = (0, lo + 2)
        else:
            new = (0, lo + 1)
...
def _encode(cmd: int, mode: int, params: List[int]) -> bytes:
    ...
    if chk == 90:  # bump id if checksum hits 90
        _next_msg_id()
        hi, lo = _last_msg_id
        body = bytes([cmd, 0x01, len(ps) + 5, hi, lo, mode, *ps])
        chk = _xor_checksum(body)
